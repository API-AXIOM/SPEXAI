"""DEPRECATED — merged into ``scripts/inference/benchmark_inference.py``.
...
(serial loop + ``.batched`` vmap groups) and ladders every current acceleration:
    ...
    python scripts/inference/benchmark_inference.py --device cuda --stages --detailed --fft-bench
...
if __name__ == "__main__":
    ...
    sys.stderr.write(f"\nrun: python "
                     f"{os.path.join(repo, 'scripts', 'inference', 'benchmark_inference.py')} "
                     "--help\n")
    sys.exit(1)
