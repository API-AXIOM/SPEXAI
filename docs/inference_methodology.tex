...
of the two came out $1.00$--$1.04$ across all four points, indistinguishable
from $1$ and far from $\sqrt{30}=5.5$: the 30 elements' emulator errors behave
as independent noise, not a coherent same-direction bias. This is a
reassuring, not a load-bearing, result -- it rules out the scenario where the
sweep below would understate the true bias by under-weighting a systematic
that pervades every element -- and it is consistent with the counts-weighted
residual figures already on record (${\sim}0.08$--$0.11\%$ here,
${\sim}0.077\%$ in Sec.~\ref{sec:hotfloor} at the Perseus fiducial).
...
\ref{tab:biassweep}, evaluated at $10^{6}$ in-band counts. Every parameter
stays below $1\sigma$ at every sampled point (\code{frac>1}$=0$ throughout);
the single worst instance across the whole sweep is $0.95\sigma$ on
$\mathrm{Fe}$, at a point with $\sigma_v=267\kms$ and a low-$\mathrm{Ca}$
($0.23\,\zsun$), high-$\mathrm{S}$/$\mathrm{Cr}$/$\mathrm{Mn}$ abundance
pattern. $kT$ carries the largest \emph{typical} offset (median
$0.33\sigma$, median $N_\star=9.0\times10^{6}$) -- still an order of
magnitude above a realistic single-region XRISM exposure
(${\sim}4\times10^{4}$) -- with every other parameter's median $N_\star$ at
least $3.6\times10^{7}$. This is a screen over 20 of the combinatorially many
points in a 12-dimensional box, not an exhaustive map; the sweep is designed
to be extended cheaply (Sec.~\ref{sec:fisherbias}'s cost argument) rather than
run once.
...
the cluster science range (single-temperature). \code{frac>1} is the fraction
of the 20 points exceeding $1\sigma$.}
\label{tab:biassweep}
\begin{tabular}{lrrrrr}
\toprule
param & median & p90 & max & frac$>$1 & median $N_\star$ \\
\midrule
Si       & 0.014 & 0.038 & 0.120 & 0.00 & $4.9\times10^{9}$ \\
S        & 0.031 & 0.069 & 0.103 & 0.00 & $1.0\times10^{9}$ \\
Ar       & 0.039 & 0.099 & 0.159 & 0.00 & $6.6\times10^{8}$ \\
Ca       & 0.057 & 0.137 & 0.183 & 0.00 & $3.2\times10^{8}$ \\
Cr       & 0.024 & 0.091 & 0.129 & 0.00 & $1.9\times10^{9}$ \\
Mn       & 0.029 & 0.093 & 0.205 & 0.00 & $1.2\times10^{9}$ \\
Fe       & 0.209 & 0.511 & 0.954 & 0.00 & $2.6\times10^{7}$ \\
Ni       & 0.112 & 0.224 & 0.255 & 0.00 & $8.2\times10^{7}$ \\
$kT$     & 0.334 & 0.513 & 0.746 & 0.00 & $9.0\times10^{6}$ \\
$\sigma_v$ & 0.056 & 0.329 & 0.715 & 0.00 & $3.2\times10^{8}$ \\
$\nh$    & 0.093 & 0.242 & 0.346 & 0.00 & $1.1\times10^{8}$ \\
$\log(\text{norm})$ & 0.168 & 0.283 & 0.508 & 0.00 & $3.6\times10^{7}$ \\
\bottomrule
\end{tabular}
\end{table}
