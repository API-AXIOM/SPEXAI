...
\paragraph{Status and open items.} The $200$k confirmation and the
higher-frequency chromium arm (\code{scripts/run\_followon\_sweep.py})
were running at the time of writing; results land in
\code{runs\_followon/sweep\_summary.md}. On confirmation the remaining
next steps are a production re-run of the full failing band (Al, S, Ar,
and re-doing Na/Ca/Cr) with \code{--film\_t\_freqs 16} as the default and
early stopping re-enabled, followed by rebuilding the shipped model store
(\code{spexai/models/}, via \code{scripts/collect\_models.py}) and
re-validating the joint inference model, whose failing-band members
predate the embedding.
